...
{
    public class AM357170355 : domain.Dalay, domain.IBaseScripts
    {
        public Hashtable startTest(Hashtable ht)
        {
            ...
            {
                //3330  输入电压（不测，未上电直接赋值）
                ec = utils.ECHelper.GetEC(ht, "3330");
                ec.Value = 0;
            ...
            if (utils.ECHelper.getFailedCount(htTestResult) == -1)
            {
                ec = utils.ECHelper.GetEC(ht, "3350");
            ...
            if (utils.ECHelper.getFailedCount(htTestResult) == -1)
            {
                ec = utils.ECHelper.GetEC(ht, "3351");
